...
\usepackage{url}
\usepackage{booktabs}
\title{EBM Transformer Compositional Grasping: ICLR Main-Conference Gate Archive}
\author{Anonymous Authors}
\begin{document}
\maketitle
\begin{abstract}
This document is not an ICLR main-conference submission. It is the v3 main-conference gate for a generated robotics draft about energy-based transformers for grasping. The original research bet was: Compose grasp energies over objects, contacts, and task constraints without collapsing them into one policy token. After a stricter hostile review, the honest terminal decision is \textbf{KILL/ARCHIVE}. Submission-hardening version: v3.
\end{abstract}
...
\begin{itemize}
\item FViT-Grasp: Grasping Objects With Using Fast Vision Transformers (2023)
\item OVAL-Grasp: Open-Vocabulary Affordance Localization for Task Oriented Grasping (2025)
\item GAT-Grasp: Gesture-Driven Affordance Transfer for Task-Aware Robotic Grasping (2025)
\item HMT-Grasp: A Hybrid Mamba-Transformer Approach for Robot Grasping in Cluttered Environments (2024)
\item FunGrasp: Functional Grasping for Diverse Dexterous Hands (2024)
\end{itemize}
